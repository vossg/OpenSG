// OpenSG Tutorial Example: MultiRangeLight
//
...
// data layout somewhat if requested. This example actively only uses point and
// spot lights and uses a simple light abstraction with only a single light
// color and one intensity value. Additionally, it provides a range attribute
// that models the attenuation behavior of the light. Of course the shader
// code's responsibilty is to make usage of the provided light data attrubutes,
// and the writer of the shader can implement whatever light model he likes
// on the given light buffer layout.
//
...
const OSG::Real32 rangeFactor       = 1.05f;
const OSG::Real32 intensityFactor   = 1.2f;
...
OSG::MultiLightChunkTransitPtr create_light_state(const VecLightsT& vLights)
{
    ...
    lightChunk->setLayoutType(OSG::MultiLight::SIMPLE_LAYOUT);
...
std::string get_fp_program()
{
    ...
    << endl << "//"
    << endl << "float calcAttenuation(in float r, in float d)"
    << endl << "{"
    ...
    << endl << "//"
    << endl << "float spotAttenuation(in float minCosSpotAngle, in vec3 l, in vec3 s)"
    << endl << "{"
