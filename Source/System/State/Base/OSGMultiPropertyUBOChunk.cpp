...
// OSGMultiPropertyUBOChunkBase.cpp file.
// To modify it, please change the .fcd file (OSGMultiPropertyUBOChunk.fcd) and
...
void MultiPropertyUBOChunk::initMethod(InitPhase ePhase)
{
...
MultiPropertyUBOChunk::MultiPropertyUBOChunk(void) :
    Inherited(),
    _bUpdateBuffer(false)
{
}

MultiPropertyUBOChunk::MultiPropertyUBOChunk(const MultiPropertyUBOChunk &source) :
    Inherited(source),
    _bUpdateBuffer(false)
{
}
...
void MultiPropertyUBOChunk::changed(ConstFieldMaskArg whichField, 
                                    UInt32            origin,
                                    BitVector         details)
{
...
void MultiPropertyUBOChunk::onCreate(const MultiPropertyUBOChunk *source)
{
...
void MultiPropertyUBOChunk::onCreateAspect(
    const MultiPropertyUBOChunk *createAspect,
    const MultiPropertyUBOChunk *source      )
{
    Inherited::onCreateAspect(createAspect, source);
}

void MultiPropertyUBOChunk::onDestroy(UInt32 uiContainerId)
{
    Inherited::onDestroy(uiContainerId);
}

/*------------------------------ Output -------------------------------------*/
void MultiPropertyUBOChunk::dump(      UInt32    ,
                                 const BitVector ) const
{
    SLOG << "Dump MultiPropertyUBOChunk NI" << std::endl;
}

/*------------------------------ buffer -------------------------------------*/

std::size_t MultiPropertyUBOChunk::calc_property_buffer_size() const
{
...
std::vector<UInt8> MultiPropertyUBOChunk::create_property_buffer(DrawEnv* pEnv) const
{
...
void MultiPropertyUBOChunk::create_property_state(DrawEnv* pEnv)
{
    std::vector<UInt8> buffer = create_property_buffer(pEnv);
    editMFBuffer()->setValues(buffer);
}

void MultiPropertyUBOChunk::update_property_state(DrawEnv* pEnv)
{
    std::vector<UInt8> buffer = create_property_buffer(pEnv);
    editMFBuffer()->setValues(buffer);
...
void MultiPropertyUBOChunk::activate(DrawEnv *pEnv, UInt32 idx)
{
    if (_bUpdateBuffer)
...
void MultiPropertyUBOChunk::deactivate(DrawEnv *pEnv, UInt32 idx)
{
...
void MultiPropertyUBOChunk::changeFrom(DrawEnv    *pEnv,
                                       StateChunk *old,
                                       UInt32      idx )
{
...
UInt32 MultiPropertyUBOChunk::addProperty()
{
    if (!check_invariant())
        clearProperties();
...
void MultiPropertyUBOChunk::removeProperty(const UInt32 idx)
{
    OSG_ASSERT(check_invariant());
...
void MultiPropertyUBOChunk::clearProperties()
{
    setNumProperties(0);
...
void MultiPropertyUBOChunk::clearMembers()
{
    clearProperties();
...
UInt32 MultiPropertyUBOChunk::addMember(Types type, UInt32 cardinality)
{
